...
- Current build allowlist: three original vectors, one WebP derivative of the existing customer logo, six WebP derivatives of selected customer/project photos, and one AI-assisted Kleinanzeigen composite.
...
- One new square lead composite combines the exact customer logo with the selected kitchen, furniture, and floor photos plus verified preview contact text. It is visibly labelled as AI-designed on `/kleinanzeigen/` and is not a separate project reference.
...
| `public/preview/kleinanzeigen/kleinanzeigen-titelbild-ki-v1.webp` | Mit der integrierten OpenAI-Bildgenerierung aus dem vorhandenen Logo sowie den freigegebenen Preview-Ablegern für U-Küche, Hochbett und Fischgrätboden gestaltet; das exakte Logo wurde anschließend technisch unverändert eingesetzt | Projektauftraggeber gemäß den anwendbaren Bedingungen des Erzeugungswerkzeugs; Rechte an den eingesetzten Ausgangsbildern weiterhin offen | nein | ja, ausschließlich PIN/noindex-Preview auf ausdrückliche Nutzeranweisung | 2026-08-20 | Quadratisches Anzeigen-Titelbild. Enthält nur Leistungen, München/Umgebung und die Preview-Telefonnummer; keine Preise, Garantien oder erfundenen Leistungsversprechen. Im UI als KI-gestaltete Collage bezeichnet. |
